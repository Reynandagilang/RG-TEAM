...
<section class="py-24 relative" style="background:#111315" id="fleet">
    ...
    <div class="max-w-7xl mx-auto px-6 relative">
        ...
        <div id="panel-{{ strtolower($catKey) }}"
             ...
             style="--car-color: {{ $color }}; --car-color-rgb: {{ $colorRgb }};">

            <div class="mb-8 p-6 rounded" style="background:#171B20; border:1px dashed rgba(255,255,255,0.06)">
                <div class="flex flex-wrap items-center gap-4 mb-3">
                    <span class="font-['Albert_Sans'] font-black text-2xl" style="color: var(--car-color)">{{ $catKey }}</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="m1-card-elevated relative overflow-hidden transition-all duration-400 hover:-translate-y-1" id="car-spec-{{ $car->id }}"
                     style="--hover-border: var(--car-color)">
                    <span class="absolute right-[-1rem] top-1/2 -translate-y-1/2 font-['Albert_Sans'] font-black text-[14rem] leading-none pointer-events-none select-none"
                          style="color: rgba(var(--car-color-rgb), 0.04)">{{ $car->car_number }}</span>

                    <div class="p-7 relative z-10">
                        <div class="flex items-start justify-between mb-6">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="font-['Albert_Sans'] font-black text-2xl" style="color: var(--car-color)">#{{ $car->car_number }}</span>
                            ...
                            @if($car->class_entry)
                            <span class="m1-badge text-xs" style="color: var(--car-color); border-color: rgba(var(--car-color-rgb), 0.2); background: rgba(var(--car-color-rgb), 0.08)">
                                {{ $car->class_entry }}
                            </span>
                            @endif
                        </div>
                        ...
                        <div class="mb-5 space-y-[1px]">
                            ...
                            <div class="flex justify-between items-center py-3 border-b border-[rgba(255,255,255,0.04)] gap-4">
                                ...
                                <span class="{{ $row['accent'] ? 'font-mono text-xs font-bold' : 'text-sm font-medium text-[#F8FAFC]' }} text-right"
                                      style="{{ $row['accent'] ? 'color: var(--car-color);' : '' }}">
                                    {{ $row['val'] ?? '—' }}
                                </span>
                            </div>
                            @endforeach
                        ...
                        <div class="space-y-3 mb-5">
                            <div>
                                <div class="flex justify-between mb-1">
                                    ...
                                    <span class="text-xs font-['Albert_Sans'] font-bold" style="color: var(--car-color)">{{ $speedPct }}%</span>
                                </div>
...
<div class="h-full rounded perf-fill" data-pct="{{ $speedPct }}"
                                          style="width:{{ $speedPct }}%; background:linear-gradient(90deg, var(--car-color), rgba(var(--car-color-rgb), 0.5));"></div>
                                </div>
                            </div>
                            <div>
                                <div class="flex justify-between mb-1">
                                    <span class="text-[#8C96A3] text-xs font-['Albert_Sans'] tracking-widest uppercase">Keluaran Tenaga</span>
                                    <span class="text-xs font-['Albert_Sans'] font-bold" style="color: var(--car-color)">{{ $powerPct }}%</span>
                                </div>
                                <div class="h-1 rounded overflow-hidden" style="background:rgba(255,255,255,0.06)">
<div class="h-full rounded perf-fill" data-pct="{{ $powerPct }}"
                                          style="width:{{ $powerPct }}%; background:linear-gradient(90deg, var(--car-color), rgba(var(--car-color-rgb), 0.5));"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-[rgba(255,255,255,0.06)] flex items-center gap-3">
                            ...
                            <span class="font-['Albert_Sans'] font-bold text-sm tracking-widest" style="color: var(--car-color)">{{ $car->livery_sponsor }}</span>
                        </div>
                        @endif
